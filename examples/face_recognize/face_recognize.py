## Copyright 2023 NXP  ##
import sensor, image, time, machine, pyb, os, tf,gc,nxp_module,struct
import lvgl as lv
import lvgl_helper
import micropython
import math
...
def msg_box_callback(obj,event):
    ...
    if event == lv.EVENT.VALUE_CHANGED:
        ...
        if obj.get_active_btn() == 0:
            ...
            ui_screen_add_face.add_style(ui_screen_add_face.PART.MAIN, style_screen_add_face_main)
            ui_screen_image_result.set_src(img_dsc)
            arr = ModelCalculateArray(face_image)
...
while(1):
    if snapstop:
        ...
        for r in objects:
            ...
            img_face = nxp_module.image_resize(img_face,64,64)
