...
mobilenet = tf.load("/sd/96_sex_cnn_quant.tflite")
clock = time.clock()
...
detnet = tf.load("/sd/face_detect_64x64.tflite")
while(1):
    icon_to = icon_to + 1
    img = sensor.snapshot()
    clock.tick()
    ...
    objects = tf.detect(detnet,img,bgr=1)
    #find faces
    if(not len(objects)):
    ...
    for r in objects:
        ...
        if (scores > 0.90):
            ...
            img = img.copy((x1,y1,w,h))
            result_img = tf.image_resize(img,96,96)
            #print("found face:%d,%d" % (w,h))
            icon_to = 0
            send2flushpreview(result_img,result_img.width(),result_img.height())
            #model classify the faces

            for obj in tf.classify(mobilenet, result_img):
                list = obj.output()
