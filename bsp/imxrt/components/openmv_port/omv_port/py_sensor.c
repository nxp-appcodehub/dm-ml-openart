...
static mp_obj_t py_sensor__init__()
{
    ...
    if (sensor_is_detected(sensor) == false) {
...
{
    struct rt_camera_device *sensor = imxrt_camera_device_find(SENSOR_NAME);
    PY_ASSERT_FALSE_MSG(sensor->ops->camera_control(sensor,RT_DRV_CAM_CMD_SLEEP,0) != 0, "Sleep Failed");
    return mp_const_none;
}
...
{
    struct rt_camera_device *sensor = imxrt_camera_device_find(SENSOR_NAME);
    PY_ASSERT_FALSE_MSG(sensor->ops->camera_control(sensor,RT_DRV_CAM_CMD_SHUTDOWN,0) != 0, "Shutdown Failed");
    return mp_const_none;
}
...
static mp_obj_t py_sensor_skip_frames(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    ...
    uint32_t millis = rt_tick_get();

    if (!n_args) {
        while ((rt_tick_get() - millis) < time) { // 32-bit math handles wrap arrounds...
            py_sensor_snapshot(0, NULL, NULL);
        }
    } else {
        for (int i = 0, j = mp_obj_get_int(args[0]); i < j; i++) {
            if ((kw_arg != NULL) && ((rt_tick_get() - millis) >= time)) {
                break;
            }
...
static mp_obj_t py_sensor_get_framerate()
{
	...
	int framerate = imxrt_camera_framerate(sensor);
	
    if (framerate == FRAMESIZE_INVALID) {
        nlr_raise(mp_obj_new_exception_msg(&mp_type_ValueError, "Frame rate not set yet!"));
    }
    return mp_obj_new_int(framerate);
...
static mp_obj_t py_sensor_set_windowing(uint n_args, const mp_obj_t *args)
{
    ...
    temp.w = resolution[framesize][0];
    temp.h = resolution[framesize][1];
    ...
    int error = imxrt_camera_set_windowing(sensor,r.x, r.y, r.w, r.h);
    if (error != 0) {
...
{
	struct rt_camera_device *sensor = imxrt_camera_device_find(SENSOR_NAME);
    PY_ASSERT_FALSE_MSG(sensor->ops->camera_control(sensor,RT_DRV_CAM_CMD_SET_AUTOROTATE,0) != 0, "set_auto_rotation Failed");
    return mp_const_none;
}
...
static mp_obj_t py_sensor_set_vsync_callback(mp_obj_t vsync_callback_obj)
{
    ...
    if (!mp_obj_is_callable(vsync_callback_obj)) {
        ...
        sensor->vsync_callback = (sensor_vsync_callback);
    }
...
static mp_obj_t py_sensor_set_frame_callback(mp_obj_t frame_callback_obj)
{
    ...
    if (!mp_obj_is_callable(frame_callback_obj)) {
		...
		sensor->frame_callback = sensor_frame_callback;
    }
    ...
    switch (request) {
        case IOCTL_SET_READOUT_WINDOW: {
            if (n_args >= 2) {
                ...
                error = imxrt_camera_ioctl(sensor, request, x, y, w, h);
            }
            break;
        }
        ...
        case IOCTL_SET_TRIGGERED_MODE: {
            if (n_args >= 2) {
                error = imxrt_camera_ioctl(sensor, request, mp_obj_get_int(args[1]));
            }
            break;
        }
        ...
        case IOCTL_GET_TRIGGERED_MODE: {
            int enabled;
            error = imxrt_camera_ioctl(sensor, request, &enabled);
            if (error == 0) {
        ...
        case IOCTL_RESET_AUTO_FOCUS: {
            error = imxrt_camera_ioctl(sensor, request);
            break;
        }
        case IOCTL_WAIT_ON_AUTO_FOCUS: {
            error = imxrt_camera_ioctl(sensor, request, (n_args < 2) ? 5000 : mp_obj_get_int(args[1]));
            break;
        }
        #endif
        ...
        case IOCTL_LEPTON_GET_WIDTH: {
            int width;
            error = imxrt_camera_ioctl(sensor, request, &width);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_GET_HEIGHT: {
            int height;
            error = imxrt_camera_ioctl(sensor, request, &height);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_GET_RADIOMETRY: {
            int radiometry;
            error = imxrt_camera_ioctl(sensor, request, &radiometry);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_GET_REFRESH: {
            int refresh;
            error = imxrt_camera_ioctl(sensor, request, &refresh);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_GET_RESOLUTION: {
            int resolution;
            error = imxrt_camera_ioctl(sensor, request, &resolution);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_SET_ATTRIBUTE: {
            if (n_args >= 3) {
                ...
                error = imxrt_camera_ioctl(sensor, request, command, data, data_len / sizeof(uint16_t));
            }
            break;
        }
        ...
        case IOCTL_LEPTON_GET_ATTRIBUTE: {
            if (n_args >= 3) {
                ...
                error = imxrt_camera_ioctl(sensor, request, command, data, data_len);
                if (error == 0) {
        ...
        case IOCTL_LEPTON_GET_AUX_TEMPERATURE: {
            int temp;
            error = imxrt_camera_ioctl(sensor, request, &temp);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_SET_MEASUREMENT_MODE:
            if (n_args >= 2) {
                ...
                error = imxrt_camera_ioctl(sensor, request, mp_obj_get_int(args[1]), high_temp);
            }
            break;

        case IOCTL_LEPTON_GET_MEASUREMENT_MODE: {
            int enabled, high_temp;
            error = imxrt_camera_ioctl(sensor, request, &enabled, &high_temp);
            if (error == 0) {
        ...
        case IOCTL_LEPTON_SET_MEASUREMENT_RANGE:
            if (n_args >= 3) {
                ...
                error = imxrt_camera_ioctl(sensor, request, &min, &max);
            }
            break;
...
mp_obj_t py_sensor_open(){

	return mp_const_none;
}

mp_obj_t py_sensor_close(){

	return mp_const_none;
}

STATIC MP_DEFINE_CONST_FUN_OBJ_0(py_sensor_open_obj,       py_sensor_open);
STATIC MP_DEFINE_CONST_FUN_OBJ_0(py_sensor_close_obj,       py_sensor_close);

STATIC const mp_map_elem_t globals_dict_table[] = {
    { MP_OBJ_NEW_QSTR(MP_QSTR___name__),            MP_OBJ_NEW_QSTR(MP_QSTR_sensor)},
    { MP_OBJ_NEW_QSTR(MP_QSTR_open),                (mp_obj_t)(&py_sensor_open_obj)},
	{ MP_OBJ_NEW_QSTR(MP_QSTR_close),                (mp_obj_t)(&py_sensor_close_obj)},
    // Pixel Formats
