...
extern char Image$$OMV_MAIN_FB_END$$Base;
#define _fb_base Image$$OMV_MAIN_FB$$Base
#define _fb_end Image$$OMV_FB_END$$Base
#else
extern int OMV_MAIN_FB;
extern int OMV_FB_END;
#define _fb_base OMV_MAIN_FB
#define _fb_end OMV_FB_END
#endif

#define JPEG_BUF	__attribute__((section(".jpeg_buf")))
JPEG_BUF uint32_t _jpeg_buf[OMV_JPEG_BUF_SIZE / 4];
...
void fb_update_jpeg_buffer(uint8_t *pixels, int w, int h, uint8_t bpp)
{
	image_t main_fb_src;
    framebuffer_init_image(&main_fb_src);
    image_t *src = &main_fb_src;
	static int overflow_count_ex = 0;
    src->pixels = pixels;
	src->w = w;
	src->h = h;
	src->bpp = bpp;
    ...
    if (src->pixfmt != PIXFORMAT_INVALID &&
        ...
        if (src->is_compressed) {
            ...
            if (does_not_fit) {
                mp_printf(MP_PYTHON_PRINTER,"Warning: JPEG/PNG too big! Trying framebuffer transfer using fallback method!\n");
            ...
            if (mutex_try_lock_alternate(&jpeg_framebuffer->lock, MUTEX_TID_OMV)) {
                ...
                if (overflow) {
                    ...
                    if ((!overflow_count_ex) && (jpeg_framebuffer->quality < jpeg_quality_max)) {
...
void framebuffer_free_current_buffer()
{
    ...
    SCB_InvalidateDCache_by_Addr((uint32_t *)buffer->data, framebuffer_get_buffer_size());
    #endif
