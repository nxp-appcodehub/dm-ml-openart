...
STATIC mp_obj_t mp_allocate_buf(mp_obj_t w_obj, mp_obj_t h_obj){
	...
	void* buf = fb_alloc(len, FB_ALLOC_PREFER_SIZE);
...
extern void fb_update_jpeg_buffer(uint8_t *pixels, int w, int h, uint16_t bpp);
...
//		MAIN_FB()->data = (uint8_t*)color_ptr;
//		//rt_enter_critical();
		fb_update_jpeg_buffer((uint8_t*)color_ptr,w,h,COLOR_DEPTH); 
		//rt_exit_critical();
