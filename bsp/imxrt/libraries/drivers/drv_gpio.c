...
static int imxrt_pin_get_mode(rt_device_t dev, rt_base_t pin)
{
    int value;
    rt_int8_t port, pin_num;
    uint32_t *cfg_reg;
    ...
    if (mask_tab[port].gpio != GPIO5)
    {
        ...
        cfg_reg = configReg_base + reg_offset[pin] * 4;			
#else 			
        cfg_reg = (uint32_t *)(CONFIG_BASE + reg_offset[pin] * 4);
#endif			
    }
    else
    {
		...
		#else
			cfg_reg = (uint32_t *)(GPIO5_CONFIG_BASE + pin_num * 4);
		#endif
	
    }

    cfg_value = REG_READ32(cfg_reg);
    gpio_base = mask_tab[port].gpio;
    uint32_t gpio_value = REG_READ32(gpio_base->GDIR);
    uint32_t direction;
    if((gpio_value & (1 << pin_num)) == 0)
    {
    ...
    {
        if(cfg_value == 0x0830U) //OD
            return PIN_MODE_OUTPUT_OD;
        else
            return PIN_MODE_OUTPUT;
    }
    else
    {
        if (cfg_value == 0xB030U)
            return PIN_MODE_INPUT_PULLUP;
        else if (cfg_value == 0x3030U)
