...
bool imx_cam_sensor_init(struct imxrt_camera *cam)
{
	...
	CLOCK_EnableClock(kCLOCK_Csi);
	cam_err("csi root clk:%d\r\n",CLOCK_GetClockRootFreq(kCLOCK_CsiClkRoot));
	for (int i=0; i< 3;i++)
	{
...
void CSI_IRQHandler(void) {
	...
	if (csisr & (1<<16)) {
		...
		if (!sync_found)
		{
			...
			pCam->s_irq.base0 = (uint32_t)buffer->data;
...
void imx_cam_csi_fragmode_calc(struct imxrt_camera *cam) {
	...
	if (cam->sensor.pixformat == PIXFORMAT_GRAYSCALE) {
		...
		cam->s_irq.isGray = 1;
		cam->sensor.hw_flags.gs_bpp = 1;
	} else {
		cam->s_irq.isGray = 0;
		cam->sensor.hw_flags.gs_bpp = 2;
	}
...
static rt_size_t imx_cam_get_frame(struct rt_camera_device *cam, image_t * image)
{
	...
	if(JPEG_FB()->enabled & (!lvgl_running))
